...
if (save_png == 1) {
  ggsave(paste0(getwd(),"/figures/figS2.png"),
         plot = e1_BFplot, width = 26, height = 20, dpi = 900, units = "cm",limitsize = T)
}
...
if (save_png == 1) {
  ggsave(paste0(getwd(),"/figures/fig2.png"),
       plot = fig2, width = 25*1.61803398875, height = 25, units = "cm",limitsize = T)
}
...
if (save_png == 1) {
  ggsave(paste0(getwd(),"/figures/figS3.png"),
         plot = e2_BFplot, width = 26, height = 20, dpi = 900, units = "cm",limitsize = T)
}
...
if (write_csv == 1) {
  ...
  write.csv(to_print_post_hoc,paste0(getwd(),"/stats/e2_postHoc_propContrast_v2.csv"))
}
...
if (save_png == 1) {
  ggsave(paste0(getwd(),"/figures/figS4.png"),
         plot = e3_BFplot, width = 26, height = 20, dpi = 900, units = "cm",limitsize = T)
}
...
if (save_png == 1) {
  ggsave(paste0(getwd(),"/figures/fig5.png"), 
...
for (s in 1:length(unique(lf_$condition))) {
  ...
  sigmParam[s,9]  <- (qnorm(h) + qnorm(f)) / 2
}
...
ggsave(paste0(getwd(),"/SDT_cellD.png"),plot=p,
       width = 18, height = 12, dpi = 900, units = "cm",limitsize = T)
